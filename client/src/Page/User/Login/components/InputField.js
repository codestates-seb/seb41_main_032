...
const InputField = ({ user, isValidInput, setUser, setIsValidInput }) => {
  ...
  return (
    <>
      <Input type="text" id="userId" placeholder="아이디를 입력해주세요." value={user.userId} onChange={handleChange} />
      {isValidInput.userId === false && <Warning>5~16글자 사이 영문이나 숫자를 입력해주세요.</Warning>}
      <Input
        type="password"
        id="password"
